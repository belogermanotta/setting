#!/bin/bash
# Backs up existing dotfiles + .obsidian, then applies the onboarding configs.
...
if [ -d "$SCRIPT_DIR/../../obsidian" ]; then
  ...
  echo "  (skip) obsidian/ not found at $SCRIPT_DIR/../../obsidian"
fi

echo ""
echo "Done. Backups (if any) are in $BACKUP_DIR"
echo "Next: git config --global user.name / user.email"
